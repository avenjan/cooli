        ...
        if(is_array($dtp->CTags))
        {
            foreach($dtp->CTags as $ctag)
            {
                if($ctag->GetName()=='link')
                {
                    ...
                    $nForm .= "<div style='line-height:36px'>下载地址{$newRowStart}：<input type='text' name='softurl{$newRowStart}' style='width:280px' value='".trim($ctag->GetInnerText())."' />
            下载显示名称：<input type='text' name='servermsg{$newRowStart}' value='".$ctag->GetAtt("text")."' style='width:150px' />
...
else if($dopost=='save')
{
    ...
    if(empty($channelid))
    {
        ShowMsg("歌曲为非指定的类型，请检查你发布内容的表单是否合法！","-1");
        exit();
    }
    ...
    $msg = "
    　　您可以继续执行以下操作：<br/>
    <a href='soft_add.php?cid=$typeid'><u>发布新歌</u></a>
    &nbsp;&nbsp;
    <a href='archives_do.php?aid=".$id."&dopost=editArchives'><u>继续修改</u></a>
    &nbsp;&nbsp;
    <a href='$arcUrl' target='_blank'><u>立即浏览</u></a>
    &nbsp;&nbsp;
    <a href='catalog_do.php?cid=$typeid&dopost=listArchives'><u>已发内容件管理</u></a>
    &nbsp;&nbsp;
    <a href='catalog_main.php'><u>网站栏目管理</u></a>
    ";
    $wintitle = "成功修改一首歌曲！";
    $wecome_info = "音乐管理::修改歌曲信息";
    $win = new OxWindow();
    $win->AddTitle("成功修改歌曲信息：");
    $win->AddMsgItem($msg);
