...
else if($open==2)
{
	...
	if((strstr($row['softlinks'],".1"))){
	ShowMsg('受保护资源，无法为您提供下载！', 'javascript:window.close();',"0",5000);
        exit();
	}
	
	if((strstr($row['softlinks'],"qiyi"))){
	ShowMsg('受合作方版权保护，无法为您提供下载！', 'javascript:window.close();',"0",5000);
        exit();
	}
    ...
    if($needRank>0 || $needMoney>0)
    {
        ...
        if(($needRank>1 && $cfg_ml->M_Rank < $needRank && $mid != $cfg_ml->M_ID))
        {
            ...
            $memberTypes[0] = "游客";
            $msgtitle = "您无法下载歌曲：{$arctitle}！";
            $moremsg = "原因：发布者设定这首歌曲需要 <font color='red'>".$memberTypes[$needRank]."</font> 才能下载，你目前是：<font color='red'>".$memberTypes[$cfg_ml->M_Rank]."</font> ！";
        ...
        if($needMoney > 0  && $mid != $cfg_ml->M_ID)
        {
            ...
            if( !is_array($row) )
            {
                ...
                {
                    $msgtitle = "您无法下载歌曲：{$arctitle}！";
                    $moremsg = "原因：发布者设定这首歌曲需要 <font color='red'>".$needMoney." 金币</font> 才能下载，你目前拥有金币：<font color='red'>".$cfg_ml->M_Money." 个</font> ！";
                    include_once(DEDETEMPLATE.'/plus/view_msg.htm');
                    exit(0);
                }
                //有足够金币，记录用户信息
                $inquery = "INSERT INTO `#@__member_operation`(mid,oldinfo,money,mtime,buyid,product,pname,sta)
                  VALUES ('".$cfg_ml->M_ID."','$arctitle','$needMoney','".time()."', 'ARCHIVE".$id."', 'archive','下载歌曲', 2); ";
                //记录定单
