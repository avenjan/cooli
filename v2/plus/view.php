...
{
    ShowMsg('内容尚未通过审核,非作者本人无权查看!', 'javascript:;');
    exit();
}
...
if($needMoney>0 || $needRank>1)
{
    ...
    */                        
    ...
    if(($needRank>1 && $cfg_ml->M_Rank < $needRank && $arc->Fields['mid']!=$cfg_ml->M_ID))
    {
        ...
        $moremsg = "这首歌曲需要 <font color='red'>".$memberTypes[$needRank]."</font> 才能访问，你目前是：<font color='red'>".$memberTypes[$cfg_ml->M_Rank]."</font> ！";
    ...
    if($needMoney > 0  && $arc->Fields['mid'] != $cfg_ml->M_ID)
    {
        ...
        if(!is_array($row))
        {
            ...
            {
                    $msgtitle = "你没有权限浏览：{$arctitle} ！";
                    $moremsg = "这首歌曲需要 <font color='red'>".$needMoney." 金币</font> 才能访问，你目前拥有金币：<font color='red'>".$cfg_ml->M_Money." 个</font> ！";
                    include_once(DEDETEMPLATE.'/plus/view_msg.htm');
                    $arc->Close();
                    exit();
            }
            else
            {
                ...
                $msgtitle = "扣金币购买阅读！";
                $moremsg = "浏览该内容需要付费！<br>这首歌曲需要 <font color='red'>".$needMoney." 金币</font> 才能访问，你目前拥有金币 <font color='red'>".$cfg_ml->M_Money." </font>个！<br>确认阅读请点 [<a href='/plus/view.php?aid=".$aid."&dopost=buy' target='_blank'>确认付点访问</a>]" ;
