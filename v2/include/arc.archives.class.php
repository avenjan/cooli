    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                ...
                $this->PreNext['pre'] = "<input type=button onclick=document.location.href='$mlink'  title=上一曲  id=up /> ";
                $this->PreNext['preimg'] = "<a href='$mlink'><img src=\"{$preRow['litpic']}\" alt=\"{$preRow['title']}\"/></a> "; 
            }
            else
            {
                $this->PreNext['pre'] = "";//"上一首：没有了 ";
            ...
            if(is_array($nextRow))
            {
                ...
                //原$this->PreNext['next'] = "下一首：<a href='$mlink'>{$nextRow['title']}</a> ";
				$this->PreNext['next'] = "<input type=button onclick=document.location.href='$mlink'  title=下一曲  id=next /> ";
                $this->PreNext['nextimg'] = "<a href='$mlink'><img src=\"{$nextRow['litpic']}\" alt=\"{$nextRow['title']}\"/></a> ";
            }
            else
            {
                $this->PreNext['next'] = "";//"下一首：没有了 ";
