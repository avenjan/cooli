...
if($fmdo=='sendMail')
{
    ...
    $mailbody .= "要通过注册，还必须进行最后一步操作，请点击或复制下面链接到地址栏完成验证：\r\n\r\n";
    $mailbody .= "{$url}\r\n\r\n";
    $mailbody .= "Power by 桦盟网络 http://www.huamengnet.com \r\n";
...
else if($fmdo=='login')
{
    ...
    if($dopost=="login")
    {
        ...
        else
        {
            ...
            {
                ShowMsg("成功登录，5秒钟后为您转向会员中心...","index.php",0,2000);
            }
            else
            {
                $gourl = str_replace('^','&',$gourl);
                ShowMsg("成功登录，现在为您转向指定页面...",$gourl,0,2000);
            }
            exit();
        }
    }
