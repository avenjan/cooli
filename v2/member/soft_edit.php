    ...
    if(isset($addRow['softlinks']) && $addRow['softlinks']!='')
    {
        ...
        if(is_array($dtp->CTags))
        {
            foreach($dtp->CTags as $ctag)
            {
                ...
                {
                    $nForm .= "下载地址".$newRowStart."：<input class='text' type='text' name='softurl".$newRowStart."'  value='".trim($ctag->GetInnerText())."' />
            显示名称：<input class='text' type='text' name='servermsg".$newRowStart."' value='".$ctag->GetAtt("text")."' style='width:150px;'  />
            <br />";
                    $newRowStart++;
                }
            }
...
else if($dopost=='save')
{
    ...
    //返回成功信息
    $msg = "　　您可以可以继续选择以下操作：<br/>
        <a href='soft_add.php?cid=$typeid'><u>发布新歌</u></a>
        &nbsp;&nbsp;
        <a href='soft_edit.php?channelid=$channelid&aid=".$aid."'><u>查看更改</u></a>
        &nbsp;&nbsp;
        <a href='$artUrl' target='_blank'><u>立即浏览</u></a>
        &nbsp;&nbsp;
        <a href='content_list.php?channelid=$channelid'><u>管理已发布歌曲</u></a>
        ";
    $wintitle = "修改成功！";
    $wecome_info = "音乐管理::更改歌曲信息";
    $win = new OxWindow();
    $win->AddTitle("成功修改内容：");
    $win->AddMsgItem($msg);
